...
void Raytracer::createImage(const std::string& output){
    ...
    for (int py = 0; py < HEIGHT; py++){
        for (int px = 0; px < WIDTH; px++){
            ...
            //if stratified is turned on
            //goes through and collects sample to statirfy the code
            if (stratified){
                ...
                //then create a vector based on that and create a ray
                direction = (-1 * magD * vecW);
                direction = direction + (x * vecU);
                direction = direction + (y * vecV);
...
void Raytracer::worldSpace(){
    //all formulas to create the world space and normalize it
...
Vector3d Raytracer::colorSet(const Ray& ray){
    //init and declares variables then calls isHit to check
    Hit hr;
    Surface* currSurface = nullptr;
